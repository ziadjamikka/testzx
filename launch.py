...
# Set UTF-8 encoding for stdout and stderr to handle Arabic characters
try:
...
def check_user_permission():
    if not keyVal():
        show_permission_error("Key file not found. Please ensure 'key.key' is in the same folder.")
    try:
        resp = requests.post("https://mangatime.vip/verify_key", json={"key": keyVal()}, timeout=10)
        resp.raise_for_status()
        data = resp.json()
        if data.get("status") != "allowed":
            show_permission_error(data.get("message", "Your account is disabled or does not have permission."))
    except Exception:
