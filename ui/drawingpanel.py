...
from qtpy.QtWidgets import QGridLayout, QPushButton, QComboBox, QSizePolicy, QBoxLayout, QCheckBox, QHBoxLayout, QGraphicsView, QStackedWidget, QVBoxLayout, QLabel, QGraphicsPixmapItem, QGraphicsEllipseItem, QSlider, QColorDialog
from qtpy.QtGui import QPen, QColor, QCursor, QPainter, QPixmap, QBrush, QFontMetrics, QIcon, QFont
...
class DrawingPanel(Widget):
    ...
    def __init__(self, canvas: Canvas, inpainter_panel: InpaintConfigPanel, *args, **kwargs) -> None:
        ...
        self.undoTool = DrawToolCheckBox()
        self.undoTool.setObjectName("DrawUndoTool")
        u_pixmap = QPixmap(32, 32)
        u_pixmap.fill(Qt.transparent)
        painter = QPainter(u_pixmap)
        font = QFont()
        font.setPointSize(26)
        font.setBold(True)
        painter.setFont(font)
        painter.setPen(QColor(30, 90, 200))
        painter.drawText(u_pixmap.rect(), Qt.AlignCenter, "U")
        painter.end()
        self.undoTool.setIcon(QIcon(u_pixmap))
        self.undoTool.setIconSize(QSize(32, 32))
        self.undoTool.setMinimumSize(QSize(40, 40))
        self.undoTool.setMaximumSize(QSize(40, 40))
        self.undoTool.setText("")
        self.undoTool.setToolTip("Undo")
        self.undoTool.setStyleSheet('''
            QCheckBox {
        ...
        ''')
        self.undoTool.setCheckable(True)
        self.undoTool.setSizePolicy(QSizePolicy.Policy.Minimum, QSizePolicy.Policy.Minimum)
        self.undoTool.checked.connect(self.on_use_undo_tool)
        self.undo_brush_width = 20  
        self.undoPanel = ExtraDrawPanel(self)
        self.undoPanel.sizeSlider.setValue(self.undo_brush_width)
        self.undoPanel.sizeSlider.valueChanged.connect(self.setUndoBrushWidth)
        ...
        toolboxlayout.addWidget(self.undoTool) 
        ...
        self.toolConfigStackwidget.addWidget(self.undoPanel)  # لوحة الأدوات الجديدة
    ...
    def setPenCursor(self):
        # تخصيص الكورسر لوضع UndoInpaintBrush
        if self.canvas.image_edit_mode == ImageEditMode.UndoInpaintBrush:
            from qtpy.QtGui import QCursor, QPixmap, QPainter, QColor
            size = self.undo_brush_width
            pixmap = QPixmap(size+8, size+8)
            pixmap.fill(Qt.transparent)
            painter = QPainter(pixmap)
            painter.setRenderHint(QPainter.RenderHint.Antialiasing)
            pen = QPen(QColor(80, 120, 200), 2)
            painter.setPen(pen)
            painter.drawEllipse(4, 4, size, size)
            painter.end()
            self.canvas.gv.setCursor(QCursor(pixmap))
            return
        # الافتراضي للأدوات الأخرى
    ...
    def on_use_undo_tool(self) -> None:
        if self.currentTool is not None and self.currentTool != self.undoTool:
            self.currentTool.setChecked(False)
        self.currentTool = self.undoTool
        self.canvas.image_edit_mode = ImageEditMode.UndoInpaintBrush
        self.toolConfigStackwidget.setCurrentWidget(self.undoPanel)
        self.setPenCursor()  # تغيير الكورسر إلى فرشاة Undo

    def setUndoBrushWidth(self, width):
        self.undo_brush_width = width
        if self.canvas.image_edit_mode == ImageEditMode.UndoInpaintBrush:
            self.canvas.painting_pen.setWidthF(width)
            # تحديث الكيرسر لو لزم الأمر
            if self.isVisible():
                self.setPenCursor()


class ExtraDrawPanel(Widget):
    def __init__(self, parent_panel=None, *args, **kwargs):
        super().__init__(*args, **kwargs)
        ...
        layout.addWidget(QLabel("Brush Type:"))
        self.typeCombo = QComboBox()
        self.typeCombo.addItems(["Normal", "Marker", "Calligraphy"])
