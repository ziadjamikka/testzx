...
def find_project_root(marker: str = "launch.py") -> Path:
...
class ImgtransThread(QThread):
    ...
    update_remake_progress = Signal(int)
    ...
    def show_permission_error(message):
        app = QtWidgets.QApplication([])
        QtWidgets.QMessageBox.critical(None, "Access Denied", message)
        sys.exit(1)
    ...
    def _imgtrans_pipeline(self):
        ...
        for imgname in self.imgtrans_proj.pages:
            ...
            if cfg_module.enable_Remake:
                ...
                scripts_dir = project_root / "gemini_client"
                package_dir = scripts_dir / "gemini_client"

                sys.path.insert(0, str(package_dir))
                sys.path.insert(0, str(scripts_dir))

                import importlib
                import process_single_page
                importlib.reload(process_single_page)

                json_file = osp.join(self.imgtrans_proj.directory, self.imgtrans_proj.proj_name() + '.json')
                image_name = imgname

               
                try:
                    # verify key
                    resp = requests.post("https://mangatime.vip/verify_key", json={"key": keyVal()})
                    resp.raise_for_status()
                    vr = resp.json()
                    if vr.get("status") != "allowed":
                        show_permission_error(vr.get("message", "Your account is disabled or does not have permission."))

                    # fetch random token
                    token_resp = requests.post("https://mangatime.vip/random_token", json={"key": keyVal()})
                    token_resp.raise_for_status()
                    token_data = token_resp.json()
                    api_key = token_data.get("token")
                    if not api_key:
                        show_permission_error(token_data.get("error", "Failed to retrieve token."))

                except Exception:
                    show_permission_error("Unable to connect to the server. Please check your internet connection.")

                process_single_page.run(
                    str(json_file),
                    image_name,
                    cfg_module.translate_source,
                    cfg_module.translate_target,
                    api_key=api_key
                )
                time.sleep(1)
                self.imgtrans_proj.refrasheAndSaveFromJsonFile(json_file)
                time.sleep(1)

                self.remake_counter += 1
                self.update_remake_progress.emit(self.remake_counter)
    ...
    def remake_finished(self) -> bool:
        if self.imgtrans_proj is None or not cfg_module.enable_Remake:
            return True
        return self.remake_counter == self.num_pages or not cfg_module.enable_Remake
    ...
    def recent_finished_index(self, ref_counter: int) -> int:
        ...
        if cfg_module.enable_Remake:
            ref_counter = min(ref_counter, self.remake_counter)
        if cfg_module.enable_translate:
            if self.parallel_trans:
...
class ModuleManager(QObject):
    ...
    def setupThread(self, config_panel: ConfigPanel, imgtrans_progress_msgbox: ImgtransProgressMessageBox, ocr_postprocess: Callable = None, translate_postprocess: Callable = None):
        ...
        self.imgtrans_thread.update_remake_progress.connect(self.on_update_remake_progress)
    ...
    def runImgtransPipeline(self):
        ...
        self.progress_msgbox.remake_bar.setVisible(cfg_module.enable_Remake)
    ...
    def on_finish_blktrans_stage(self, stage: str, progress: int):
        if stage == 'ocr':
            ...
            self.progress_msgbox.updateRemakeProgress(progress)
        else:
    ...
    def on_update_remake_progress(self, progress: int):
        ri = self.imgtrans_thread.recent_finished_index(progress)
        progress = int(progress / self.imgtrans_thread.num_pages * 100)
        self.progress_msgbox.updateRemakeProgress(progress)
        if ri != self.last_finished_index:
            ...
            and self.imgtrans_thread.ocr_finished() \
                and self.imgtrans_thread.translate_finished() \
                    and self.imgtrans_thread.inpaint_finished()\
                        and self.imgtrans_thread.remake_finished():
            self.progress_msgbox.hide()
            self.imgtrans_pipeline_finished.emit()
